...
//Note: This terrain grid will eventually be created/modified
// 		in the game editor.


//This terrain grid contains indices to quads making up the terrain
//used to compare quad tiles to other game content such as tag blocks
//and sprites
int terraingrid[TILE_GRID_MAX_WIDTH][TILE_GRID_MAX_HEIGHT];

//a quad tile contains two indices into the triangle list
//one for each triangle making up the quad
typedef struct quad_tile {
	int triangle_a_index; //index into triangle list for left half of quad (x least)
	int triangle_b_index; //index into triangle lsit for right half of quad (x most)
}quad_tile;

//list of quads, indexed by the terraingrid
quad_tile quad_tiles[TILE_GRID_MAX_WIDTH*TILE_GRID_MAX_HEIGHT*2]; //list of quad tiles



//the number of triangles in the triangle/vertex list
//number of vertices is number_of_triangles*3, for 3 vertex per
//triangle
int number_of_triangles;

//3 vertices per triangle, MAX_TERRAIN_TRIANGLES is a constant
//related to the max number of triangles allowed for the level
//terrain.
ALLEGRO_VERTEX v[MAX_TERRAIN_TRIANGLES][3];


//renders a 3D terrain underneath everything
...
void get_display() //gets the display bitmaps
{
	...
	al_set_new_display_flags(ALLEGRO_WINDOWED|ALLEGRO_OPENGL);
	 ...
	 if(!display) {
	    ...
	    // I added this because otherwise you'll get a full screen white
	    // before it adjusts and stretches the display momentarily.
...
//DEPRICATED - gamerenderer() used by game
void renderer(void)
{
...
void create_generic_terrain()
{
	//set
	//Reference: ALLEGRO_VERTEX v[3][MAX_TERRAIN_TRIANGLES];

	//loop through triangles, k=triangle
	for (int k = 0; k<MAX_TERRAIN_TRIANGLES; k++) {

		/*for (int vert = 0; vert < 3; vert++) {
			v[k][vert].x = k*10*vert;
			v[k][vert].y = k*10*vert;
			v[k][vert].z = 1;
			v[k][vert].u = k*10*vert;
			v[k][vert].v = k*10*vert;
			v[k][vert].color = WHITE_COLOR;
		...
		*/
		...
		v[k][0].y = 10.0f;
		v[k][0].z = 10.0f;
		v[k][0].color = WHITE_COLOR;

		v[k][1].x = 20.0f;
		v[k][1].y = 10.0f;
		v[k][1].z = 10.0f;
		v[k][0].color = GREEN_COLOR;


		v[k][2].x = 10.0f;
	    v[k][2].y = 20.0f;
	    v[k][2].z = 20.0f;
		v[k][0].color = RED_COLOR;


		number_of_triangles++; //increment number of triangles ctr in list
	}


	//create quad list
	//loop through triangles, ctr=quad index

	int quad_ctr = 0;

	for (int i = 0; i<MAX_TERRAIN_TRIANGLES; i+= 2) {
		//create triangle
		quad_tile quad;

		quad.triangle_a_index = i;
		quad.triangle_b_index = i +1;

		quad_tiles[quad_ctr] = quad;
		quad_ctr++;
	}

	//initialize terrain grid with quad indices
	quad_ctr = 0;

	//int terraingrid[TILE_GRID_MAX_WIDTH][TILE_GRID_MAX_HEIGHT];


	for (int i=0;i<TILE_GRID_MAX_WIDTH;i++) {
		for (int j=0;j<TILE_GRID_MAX_HEIGHT;j++) {
			terraingrid[i][j] = quad_ctr;
			quad_ctr++;
		}
	}
...
void render_terrain_grid()
{
	//generic renderer renders the triangle list with
	//green coloring

	logger("Calling render_Terrain_grid");
	logger("Number of Triangles is");
	logint(number_of_triangles);
	//for (int i =0; i<number_of_triangles;i++)
//	{
		al_draw_prim(v, NULL, floor1, 0, 3*number_of_triangles,
